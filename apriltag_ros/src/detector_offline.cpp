...
DetectorOffline::DetectorOffline (ros::NodeHandle nh, ros::NodeHandle nh_private) : nh_(nh), nh_private_(nh_private)
{
    ...
    draw_tag_detections_image_ = getAprilTagOption<bool>(nh_private_, 
      "publish_tag_detections_image", false);
    it_ = std::shared_ptr<image_transport::ImageTransport>(
      new image_transport::ImageTransport(nh_));
    tag_detections_publisher_ =
        nh_.advertise<AprilTagDetectionArray>("tag_detections", 1);
...
int DetectorOffline::ReadImage()
{
    ...
    cv::Mat camera_matrix = (cv::Mat_<double>(3,3)<<camera_.fx, 0.0, camera_.cx, 0.0, camera_.fy, camera_.cy, 0.0, 0.0, 1.0); 
